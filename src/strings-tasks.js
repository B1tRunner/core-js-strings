...
function sumOfCodes(str) {
  let sum = 0;
  if (sum.length <= 0 || typeof str !== 'string') {
    return 0;
  }
...
function countVowels(str) {
  ...
  return vowels !== null ? vowels.join('').length : 0;
}
...
function isPalindrome(str) {
  const nakedStr = str.toLowerCase().replace(/[\W_]/gi, '');
  return nakedStr === nakedStr.split('').reverse().join('');
}
